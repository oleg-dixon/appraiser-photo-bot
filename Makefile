...
venv: check-python-version ## Создать виртуальное окружение через uv
	...
	@if [ ! -d ".venv" ]; then \
		...
		echo "$(YELLOW)Активируйте:$(NC)$(GREEN)source .venv/bin/activate$(NC)\n"; \
	fi
	@printf "$(CYAN)━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━$(NC)\n"
...
install: check-python-version ## Установить зависимости
	...
	@if command -v $(UV) > /dev/null; then \
		echo "$(YELLOW)🔍 ПРОВЕРКА UV.LOCK$(NC)"; \
		if [ -f "uv.lock" ]; then \
			echo "$(GREEN)  ✓ uv.lock найден$(NC)"; \
			if $(UV) lock --check > /dev/null 2>&1; then \
				echo "$(GREEN)  ✓ uv.lock валиден и соответствует pyproject.toml$(NC)"; \
				printf "$(CYAN)━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━$(NC)\n"; \
				LOCK_VALID=true; \
			else \
				echo "$(YELLOW)  ⚠️ uv.lock требует обновления$(NC)"; \
				LOCK_VALID=false; \
			fi; \
		else \
			echo "$(YELLOW)  ⚠️ uv.lock не найден, будет создан$(NC)"; \
			LOCK_VALID=false; \
		fi; \
		echo ""; \
		\
		echo "$(YELLOW)⚙️ СИНХРОНИЗАЦИЯ ОКРУЖЕНИЯ$(NC)"; \
		if [ "$$LOCK_VALID" = "true" ]; then \
			echo "$(GREEN)  Использую существующий uv.lock для синхронизации...$(NC)"; \
			$(UV) sync -q; \
		else \
			echo "$(YELLOW)  Обновляю uv.lock и синхронизирую...$(NC)"; \
			$(UV) sync --frozen; \
		fi; \
		\
		if [ $$? -eq 0 ]; then \
			echo "$(GREEN)✅ Окружение успешно синхронизировано$(NC)"; \
		else \
			echo "$(RED)❌ Ошибка при синхронизации$(NC)"; \
			exit 1; \
		fi; \
	else \
		echo "$(YELLOW)uv не найден, использую pip...$(NC)"; \
		$(VENV_DIR)/bin/python -m pip install -e .; \
		echo "$(GREEN)✅ Зависимости установлены через pip$(NC)"; \
	fi
	
	@printf "$(CYAN)━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━$(NC)\n\n"
	
	@printf "$(GREEN)📦 ПРОЕКТ ГОТОВ К ИСПОЛЬЗОВАНИЮ!$(NC)\n"
	@printf "$(CYAN)━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━$(NC)\n"
